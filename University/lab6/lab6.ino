...
static OneWire s_oneWire(A1);
static DallasTemperature s_tempSensors(&s_oneWire);

static bool isGreenPressed;
static bool isRedPressed;

void task_1() {
  static float minOut = 10000;
  static float maxOut = -10000;

  s_tempSensors.requestTemperatures();
  float tempIn = s_tempSensors.getTempCByIndex(1);
  float tempOut = s_tempSensors.getTempCByIndex(0);

  char buffer[40];
  sprintf(buffer, "IN %3s OUT %3s", String(tempIn, 1).c_str(), String(tempOut, 1).c_str());
  s_lcd.setCursor(0, 0);
  s_lcd.print(buffer);
  sprintf(buffer, "MI %3s MAX %3s", String(minOut, 1).c_str(), String(maxOut, 1).c_str());
  s_lcd.setCursor(0, 1);
  s_lcd.print(buffer);
  if (tempOut > maxOut) maxOut = tempOut;
  else if (tempOut < minOut) minOut = tempOut; 

  int red = tempOut > 27 ? 100 : 0;
  int green = (tempOut <= 27) && (tempOut >= 26) ? 100 : 0;
  int blue = tempOut < 26 ? 100 : 0;
  s_ledRGB.ChangeColorPercentage(red, green, blue);
  s_lcd.setCursor(0, 0);
}
